...
def render() -> None:
    ...
    available_files = [
        (snapshot_label, REPO_ROOT / snapshot_path)
        for snapshot_label, snapshot_path in IATI_SNAPSHOTS.items()
        if (REPO_ROOT / snapshot_path).exists()
    ]
    if not available_files:
        st.info("No se encontraron bases de datos IATI para descargar.")
        return

    for snapshot_label, file_path in available_files:
        df = _load_parquet(str(file_path))
        prepared_df = _prepare_dataframe(df)

        st.subheader(snapshot_label)
        st.caption(f"{file_path.name} · Filas: {len(df):,} · Columnas: {len(df.columns)}")
